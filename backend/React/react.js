...
router.get('/getMyReaction', (req, res) => {
    maria.query(`SELECT * FROM reaction WHERE member_num = ${req.query.memberNum} AND board_num = ${req.query.boardNum}`, (err, rows) => {
        if(!err){
...
router.post('/insertMyReaction', (req, res) => {
    maria.query(`INSERT INTO reaction (reaction_type, member_num, board_num) VALUES (${req.body.reactionType}, ${req.body.memberNum}, ${req.body.boardNum})`, (err, rows) => {
...
router.post('/updateMyReaction', (req, res) => {
    maria.query(`UPDATE reaction SET reaction_type = ${req.body.reactionType} WHERE member_num = ${req.body.memberNum} AND board_num = ${req.body.boardNum} `, (err, rows) => {
        if(!err) res.send(true);
        else {
            console.log(err);
            res.send(false);
...
router.get('/getReaction', (req, res) => {
    maria.query(`SELECT reaction_type, COUNT(*) AS reaction_count FROM reaction WHERE board_num = ${req.query.boardNum} GROUP BY reaction_type`, (err, rows) => {
        if(!err){
