<template>
    <div v-if="boardDetail" class="anony-board-detail-section">
        ...
        <AnonyBoardButton v-if="buttonShowState" @reactionType="reactType" :myReact="myReact" ref="childFunction" :reactionCountVO="reactionCountVO" />
    </div>
</template>
<script setup>
/**
...
async function getReaction(){
    ...
    .then(res => {
        ...
        console.log('반응 데이터 가공 : ' + reactionCountVO.value.get(1));
    })
    .catch(error => console.log(error));
}
...
async function getMyReaction(){
    await axios.get('/api/react/getMyReaction',{
        params: {
            boardNum : boardNum,
...
async function updateReaction(type){
    ...
    .then(res => {
        if(res.data) {
            getMyReaction();
            getReaction();
            console.log('데이터 통신 후 ' + myReact.value.reactionType + '데이터 통신하기 전 바뀐 타입' + type);
